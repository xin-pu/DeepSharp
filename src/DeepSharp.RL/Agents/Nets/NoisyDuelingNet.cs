...
public sealed class NoisyDuelingNet : Module<torch.Tensor, torch.Tensor>
{
    ...
    public NoisyDuelingNet(long obsSize, long hiddenSize, long actionNum,
    ...
    public override torch.Tensor forward(torch.Tensor input)
    {
        ...
        var v = torch.nn.functional.relu(_fcValue.forward(x));
        v = _valueOut.forward(v); // [batch, 1]

        var a = torch.nn.functional.relu(_fcAdvantage.forward(x));
        a = _advantageOut.forward(a); // [batch, actionSize]

        // Q(s,a) = V(s) + A(s,a) - mean(A)
        return v + (a - a.mean(new long[] { 1 }, keepdim: true));
    }
