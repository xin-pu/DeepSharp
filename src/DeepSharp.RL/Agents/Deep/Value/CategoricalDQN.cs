...
public class CategoricalDQN : DeepValueAgent
{
    ...
    public CategoricalDQN(Environ<Space, Space> env,
        int   n         = 1000,
        int   c         = 10000,
        float gamma     = 0.99f,
        int   batchSize = 32,
        ...
        : base(env, "CategoricalDQN")
    {
        ...
        Q       = new CategoricalNet(ObservationSize, 128, ActionSize, NumAtoms, DeviceType.CPU);
        QTarget = new CategoricalNet(ObservationSize, 128, ActionSize, NumAtoms, DeviceType.CPU);
    ...
    private torch.Tensor GetQValues(torch.Tensor logits)
    {
        ...
        var probs = torch.nn.functional.softmax(logits, dim: -1); // [batch, actionSize, numAtoms]
        return (probs * Support).sum(dim: new long[] { -1 });      // [batch, actionSize]
    }
    ...
    private float UpdateNet()
    {
        ...
        var predProbs   = torch.nn.functional.softmax(predLogits, dim: -1);
        ...
        var targetProbs  = torch.nn.functional.softmax(targetLogits, dim: -1);
        var targetQ      = (targetProbs * Support).sum(dim: new long[] { -1 }); // [batch, actionSize]

        // a* = argmax_a QTarget(s', a)
        var bestActions = targetQ.argmax(dim: 1); // [batch]
        ...
        // Project target distribution onto discrete support points (manual loop, batch usually ≤256)
        ...
        var loss = -(targetDistribution * (predictedDist + 1e-8f).log()).sum(dim: 1).mean();
