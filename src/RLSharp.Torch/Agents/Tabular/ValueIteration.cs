...
namespace RLSharp.Torch.Agents.Tabular
{
	...
	{
		public ValueIteration(EnvironmentBase<Space, Space> env, int t, float gamma = 0.9f)
		...
		/// <summary>
		///     Select action according to latest ObservationValue.
		/// </summary>
		...
		public override ActionValue GetPolicyAct(torch.Tensor state)
		{
			...
			var valueDict = actionSpace
				.ToDictionary(a => a.ActionValue, a => GetActionValue(a));
		...
		public override void Load(string path)
		{
			...
			Rewards = data.Rewards.ToDictionary(
				r => new RewardKey(
					new ObservationValue(torch.tensor(r.State)),
					new ActionValue(torch.tensor(r.ActionValue)),
					new ObservationValue(torch.tensor(r.NewState))),
				r => new Reward(r.Reward));
			Transits = data.Transits.ToDictionary(
				t => new TransitKey(torch.tensor(t.State), torch.tensor(t.ActionValue)),
		...
		private void UpdateTables(ObservationValue state, ActionValue ActionValue, ObservationValue newState, Reward reward)
		{
			...
			var transitsKey = new TransitKey(state, ActionValue);
			var existTransitKey = Transits.Keys.Where(a =>
					a.ActionValue.Equals(ActionValue.Value!) &&
		...
		private float GetActionValue(TransitKey transitKey)
		{
			...
			{
				var reward = getReward(new RewardKey(transitKey.State, transitKey.ActionValue, i.Key));
		...
		private Dictionary<torch.Tensor, int> getTransit(TransitKey traitKey)
		{
			try
			{
				var key = Transits.Keys
					.First(a => a.ActionValue.Equals(traitKey.ActionValue) &&
		...
		private float getValue(torch.Tensor ObservationValue)
		{
			try
			{
				var key = Values.Keys.First(a => a.Equals(ObservationValue));
				return Values[key];
			}
			catch
			{
