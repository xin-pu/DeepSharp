...
namespace RLSharp.Torch.Agents
{
	...
	public class QTable : IEquatable<QTable>
	{
		...
		/// <summary>
		///     argmax_{a} Q(state, a) ¡ª returns the best action, or null if Q is zero.
		/// </summary>
		/// <param name="state"></param>
		/// <returns></returns>
		public ActionValue? GetBestAct(torch.Tensor state)
		{
			var row = TransitKeys
			...
			var argMax = stateActions
				.MaxBy(a => a.Value);
			var ActionValue = argMax.Key.ActionValue;
			return new ActionValue(ActionValue);
		}

		/// <summary>
		///     max_{a} Q(state, a) ¡ª returns the best Q value.
		/// </summary>
		/// <param name="state"></param>
		/// <returns></returns>
