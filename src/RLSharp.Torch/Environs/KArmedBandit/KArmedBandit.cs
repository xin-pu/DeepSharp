...
namespace RLSharp.Torch.Environs
{
	...
	/// </summary>
	public sealed class KArmedBandit : EnvironmentBase<Space, Space>
	{
		...
		/// <summary>
		///     Sum the ObservationValue tensor as the reward (number of coins from all bandits).
		/// </summary>
		/// <param name="ObservationValue"></param>
		/// <returns></returns>
		public override Reward GetReward(ObservationValue ObservationValue)
		{
			var sum = ObservationValue.Value!.to_type(torch.ScalarType.Float32)
				.sum()
				.item<float>();
		...
		/// <summary>
		/// </summary>
		/// <param name="ActionValue">Action containing the selected bandit arm index.</param>
		/// <returns>ObservationValue with 0 or 1 for the selected arm.</returns>
		public override ObservationValue Update(ActionValue ActionValue)
		{
			var obs    = new float[ObservationSpace!.N];
			var index  = ActionValue.Value!.ToInt64();
			var bandit = bandits[index];
			var value  = bandit.Step();
			obs[index] = value;

			var obsTensor = torch.from_array(obs, torch.ScalarType.Float32).to(Device);
			return new ObservationValue(obsTensor);
		}
