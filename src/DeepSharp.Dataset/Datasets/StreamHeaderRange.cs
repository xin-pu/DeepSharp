...
namespace DeepSharp.Dataset
{
	...
	public sealed class StreamHeaderRange
	{
		...
		{
			Debug.Assert(index >= 0, "Must be non-negative");
			Min = index;
			Max = index;
		}
		...
		{
			Debug.Assert(min >= 0, "Must be non-negative");
			Debug.Assert(max >= min, "If specified, must be greater than or equal to " + nameof(min));
